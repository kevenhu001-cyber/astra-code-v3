...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        // Windows has no /dev/null or /dev/stdout, so probe with temp files
        // there and read the dependency file back instead of stdout.
        let probe_dir = if cfg!(windows) {
            Some(tempfile::TempDir::new().context("create tempdir for protoc probes")?)
        } else {
            None
        };
        ...
        for proto in protos {
            ...
            match &probe_dir {
                Some(dir) => {
                    dependency_path = dir.path().join("deps.txt");
                    command
                        .arg(format!(
                            "--dependency_out={}",
                            dependency_path.to_str().context("probe path not UTF-8")?
                        ))
                        .arg(format!(
                            "--descriptor_set_out={}",
                            dir.path()
                                .join("descriptor.pbbin")
                                .to_str()
                                .context("probe path not UTF-8")?
                        ));
                }
                None => {
                    command
            ...
            for line in iter::once(rem).chain(lines) {
                ...
                // Match both / and \ separators (Windows uses the latter).
