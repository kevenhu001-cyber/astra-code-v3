...
//!   fetch version → download astra binary → chmod → atomic symlink →
//!   cleanup_old_downloads → persist installer config.
//!
//! The function reads `astra_home()` (a process-wide `OnceLock`), so all
//! tests in this binary share a single `ASTRA_HOME` and run serially via
//! `#[serial]`.
...
async fn mount_gcs(version: &str, platform: &str) -> MockServer {
    ...
    // Main astra binary download.
    Mock::given(method("GET"))
        .and(path(format!("/astra-{version}-{platform}")))
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let symlink = home.join("bin").join("astra");
    assert!(symlink.is_symlink(), "grok symlink created");
    let target = std::fs::read_link(&symlink).unwrap();
    assert_eq!(
        target.file_name().unwrap(),
        format!("astra-0.1.181-{platform}").as_str()
    );

    // `astra` and `agent` move together — see `swap_managed_bin_links`.
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    let old_binary = download_dir.join(format!("astra-0.1.180-{platform}"));
...
/// Rollback regression: if `agent` swap fails after `astra` succeeded,
/// `astra` must roll back to its prior target (all-or-nothing).
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_grok_when_agent_swap_fails() {
    ...
    let grok_target = std::fs::read_link(bin_dir.join("astra")).unwrap();
    assert_eq!(
        grok_target.file_name().unwrap(),
        format!("astra-0.1.180-{platform}").as_str(),
        "grok must be rolled back when agent swap fails"
    );
}

/// Absent-prior rollback regression: fresh install (no prior `astra` /
/// `agent`), sabotaged `agent` swap must *remove* the just-created `astra`
/// link so we don't leave it on the new binary while `agent` is absent.
#[tokio::test]
#[serial]
async fn install_internal_rollback_removes_absent_prior_astra_link() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // No prior `astra`. Sabotage `agent` swap: non-empty directory → EISDIR.
    let agent_dir = bin_dir.join("agent");
    std::fs::create_dir(&agent_dir).unwrap();
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("astra").exists() && !bin_dir.join("astra").is_symlink(),
    ...
    let grok_path = bin_dir.join("astra");
    assert!(
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    ...
    // update removes the stale symlink from ~/.astra/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    assert!(
        downloads.join(format!("astra-0.1.181-{platform}")).exists(),
        "current"
    );
    assert!(
        downloads.join(format!("astra-0.1.180-{platform}")).exists(),
        "N-1 retained"
    );
    assert!(
        !downloads.join(format!("astra-0.1.179-{platform}")).exists(),
        "oldest deleted"
    );

    // Symlink updated to latest.
    let target = std::fs::read_link(home.join("bin").join("astra")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("astra")).unwrap();
