...
//! its own `OnceLock<ASTRA_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the ASTRA_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes ASTRA_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// ASTRA_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `ASTRA_HOME`, initialized exactly once per test
/// binary. Once initialized, `xai_grok_config::astra_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("ASTRA_HOME", &path);
            std::env::remove_var("GROK_TEST_VERSION");
            std::env::remove_var("NPM_TOKEN");
            std::env::remove_var("ASTRA_INSTALLER");
            std::env::remove_var("ASTRA_MANAGED_BY_NPM");
            std::env::remove_var("ASTRA_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `ASTRA_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_astra_version()` for the
...
/// Backdate every file in `ASTRA_HOME/downloads` by ~2 hours.
///
