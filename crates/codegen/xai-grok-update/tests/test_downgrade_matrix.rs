...
async fn mount_gcs_with_channels(
    ...
    Mock::given(method("GET"))
        .and(path(format!("/astra-{binary_version}-{platform}")))
...
async fn internal_install_stable_rollback_0_2_7_to_0_2_5() {
    ...
    let downloaded = home
        .join("downloads")
        .join(format!("astra-0.2.5-{platform}"));
...
async fn internal_install_stable_upgrade_0_2_5_to_0_2_7() {
    ...
    let symlink = test_home().join("bin").join("astra");
    let target = std::fs::read_link(&symlink).unwrap();
...
async fn internal_install_rollback_then_upgrade_sequence() {
    ...
    assert!(
        downloads.join(format!("astra-0.2.8-{platform}")).exists(),
        "current"
    );
    assert!(
        downloads.join(format!("astra-0.2.7-{platform}")).exists(),
        "N-1 by semver"
    );
    assert!(
        !downloads.join(format!("astra-0.2.5-{platform}")).exists(),
        "lowest cleaned up"
    );
}
...
async fn internal_install_alpha_rollback_pointer_resolves_correctly() {
    ...
    Mock::given(method("GET"))
        .and(path(format!("/astra-0.2.8-alpha.1-{platform}")))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(b"#!/bin/sh\nexit 0\n".to_vec()))
        .mount(&server)
        .await;
    ...
    let downloaded = test_home()
        .join("downloads")
        .join(format!("astra-0.2.8-alpha.1-{platform}"));
    assert!(
        downloaded.exists(),
...
async fn internal_install_alpha_user_gets_newer_stable_after_stable_passes_alpha() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path(format!("/astra-0.2.7-{platform}")))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(b"#!/bin/sh\nexit 0\n".to_vec()))
        .mount(&server)
        .await;
...
// explicit `astra update`) must decide staleness from the on-disk install, not
...
async fn npm_user_upgraded_then_stable_rolled_back_stays_on_newer() {
    // User ran `astra update` and got 0.2.7. Then stable was rolled back to
