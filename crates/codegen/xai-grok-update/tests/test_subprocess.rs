...
async fn fetch_npm_tag_passes_pkg_and_tag_to_npm() {
    ...
    assert!(args.contains("@topodrive-ai-official/grok"), "args: {args}");
...
async fn fetch_npm_tag_alpha_appends_at_alpha_suffix() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@topodrive-ai-official/grok@alpha"),
        "args: {}",
        log[0]
    );
}
...
async fn install_npm_falls_back_to_dist_tag_on_no_target() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@topodrive-ai-official/grok@latest"),
...
async fn fetch_gh_release_passes_repo_flag() {
    ...
    assert!(
        log[0].contains("topodrive-ai-shared/grok-build"),
        "args: {}",
        log[0]
    );
}
