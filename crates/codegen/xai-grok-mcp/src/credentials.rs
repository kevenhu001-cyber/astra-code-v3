...
//!
//! Credentials are stored in `$ASTRA_HOME/mcp_credentials.json`, keyed by a
...
/// File name for the credential store inside `$ASTRA_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$ASTRA_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$ASTRA_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user grok home (set $ASTRA_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$ASTRA_HOME/mcp_credentials.json`.
...
mod tests {
    ...
    /// `$ASTRA_HOME/mcp_credentials.json`. Existing credential files must keep
