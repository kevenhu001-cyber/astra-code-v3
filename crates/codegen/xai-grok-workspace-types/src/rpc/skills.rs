...
pub enum SkillScope {
    /// cwd/.astra/skills
    Local,
    /// repo_root/.astra/skills
    Repo,
    /// ~/.astra/skills
    User,
    /// ~/.astra/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.astra/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.astra/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.astra/plugins/infra-plugin",
            "plugin_data": "/root/.astra/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
