...
//! preference domain `ai.x.astra` (`requirements_toml_base64`). Only admin-*forced*
...
#[cfg(target_os = "macos")]
const MANAGED_PREFERENCES_DOMAIN: &str = "ai.x.astra";
#[cfg(target_os = "macos")]
...
pub const MDM_REQUIREMENTS_SOURCE: &str = "ai.x.astra:requirements_toml_base64";
...
fn read_forced_requirements() -> Option<String> {
    ...
    // per-user domain, which a local user can set (`defaults write ai.x.astra`)
