//! Headless markdown analysis sharing Astra's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `xai-grok-markdown` so analysis matches what Astra
//! Build actually renders 1:1.
//!
//! After parsing, Astra applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Astra uses to render markdown.
///
...
/// Offset event stream from Astra's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Astra's options; count elements and flag structural issues.
