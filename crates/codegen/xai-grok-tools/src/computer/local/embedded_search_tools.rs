...
//! (release builds, self-extracted to `~/.astra/vendor/<name>-<ver>-<target>`) →
//! `~/.astra/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
// (release pipeline). Self-extracted to `~/.astra/vendor` on first use, mirroring
...
/// Write embedded `bytes` to `~/.astra/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.astra/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// bytes self-extract under `~/.astra/vendor` and the extracted `bfs` runs.
