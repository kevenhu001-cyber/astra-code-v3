//! LSP server configuration from `.astra/lsp.json`.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let project_path = cwd.join(".astra").join("lsp.json");
...
/// Load LSP server configs from `~/.astra/lsp.json` and `<cwd>/.astra/lsp.json`.
