...
///
/// `skills` is the standard layout (`.astra/skills/`, `.claude/skills/`,
...
/// name (e.g. `~/.astra/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Astra.
...
/// `cwd` (exclusive). At each directory, checks for `.astra/skills/`,
...
pub fn discover_skills_for_paths(
    ...
    let mut config_dir_names: Vec<&str> = vec![".astra", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.astra is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.astra/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.astra/ → kept (user content).
        let grok_shell = tmp.path().join(".astra").join("skills").join("shell");
        ...
        assert!(skills[0].path.contains("/.astra/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        .unwrap();
        let grok_skill = sub.join(".astra").join("skills").join("grok-dyn");
