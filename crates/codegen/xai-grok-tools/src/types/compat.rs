...
//!
//! Historically the agent hard-coded the dir lists `[".astra", ".agents",
...
impl CompatCell {
    ...
    /// Whether Astra currently implements this compatibility surface.
    ///
...
impl CompatConfig {
    ...
    ///
    /// Replaces the hard-coded `[".astra", ".agents", ".claude", ".cursor"]`
    ...
    /// Subdirectories scanned for `*.md` rules files. `.astra/rules` is always
...
mod tests {
    ...
    fn skill_config_dirs_all_on_matches_legacy_constant() {
        // Historical constant was `[".astra", ".agents", ".claude", ".cursor"]`.
        assert_eq!(
            CompatConfig::default().skill_config_dirs(),
            vec![".astra", ".agents", ".claude", ".cursor"]
        );
    }
    ...
    fn skill_config_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.skill_config_dirs(), vec![".astra", ".agents", ".claude"]);

        c.claude.skills = false;
        assert_eq!(c.skill_config_dirs(), vec![".astra", ".agents"]);

        // Only the `cursor` cell on (`claude` off): `cursor` still appended last.
        let mut c2 = CompatConfig::default();
        c2.claude.skills = false;
        assert_eq!(c2.skill_config_dirs(), vec![".astra", ".agents", ".cursor"]);
    }

    #[test]
    fn rules_dirs_all_on_matches_legacy_constant() {
        // Historical `RULES_DIRS` was `[".astra/rules", ".claude/rules", ".cursor/rules"]`.
        assert_eq!(
            CompatConfig::default().rules_dirs(),
            vec![".astra/rules", ".claude/rules", ".cursor/rules"]
        );
    }
    ...
    fn rules_dirs_gates_each_vendor() {
        ...
        assert_eq!(c.rules_dirs(), vec![".astra/rules", ".claude/rules"]);
        c.claude.rules = false;
        assert_eq!(c.rules_dirs(), vec![".astra/rules"]);
    }
