//! Canonical external-settings tool name ↔ Astra tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`xai-grok-hooks`) needs the
//! Astra tool **names** an external settings term maps to (and the reverse, for regex
...
/// One Claude tool's correspondence to Astra, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Astra [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Astra tool names this Claude tool maps to (empty when there is no direct
    /// Astra tool — the entry then only contributes a `kind`).
...
/// The Astra [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Astra tool names a Claude matcher term fires on.
...
/// Every distinct Astra name the table references, for the `xai-grok-agent` drift-check
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Astra name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
