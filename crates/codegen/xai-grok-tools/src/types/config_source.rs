...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.astra/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.astra/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.astra/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.astra/ or ~/.claude/.
    User { path: PathBuf },
