...
//! after every tool call to check for SKILL.md files in `.astra/skills/`,
...
/// Directories that contain skill definitions (`.astra/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".astra", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.astra/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.astra/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.astra/) are outside the git root so the walk breaks early.
