...
/// root `/repo/.astra-snapshots/wt-123`, this returns
/// `/workspace/repo/.astra-snapshots/wt-123`.
...
pub fn is_btrfs_subvolume(path: &Path) -> Result<Option<BtrfsInfo>> {
    ...
    // IS on btrfs), but the snapshot destination (e.g. `~/.astra/worktrees/...`)
...
mod tests {
    ...
    fn test_resolve_via_subvol_mount_nested_path() {
        // Simulates: target root /repo/.astra-snapshots/wt-123 resolved via
        // mount with root=/repo at /workspace/repo
        let mountinfo =
            "8267 8961 0:813 /repo /workspace/repo rw,relatime - btrfs /dev/loop0 rw,ssd";
        let result =
            resolve_via_subvol_mount("/dev/loop0", "/repo/.astra-snapshots/wt-123", mountinfo)
                .unwrap();
        assert_eq!(
            result,
            Some(PathBuf::from("/workspace/repo/.astra-snapshots/wt-123"))
        );
    }
