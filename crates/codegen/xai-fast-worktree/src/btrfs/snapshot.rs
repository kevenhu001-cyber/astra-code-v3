...
pub fn create_snapshot_with_symlink(btrfs_info: &BtrfsInfo, dest: &Path) -> Result<SnapshotResult> {
    ...
    // contains an empty .astra-snapshots/ directory (btrfs excludes nested subvolumes
    // from snapshots, leaving only empty directory placeholders). Remove it to
    // keep the worktree clean.
    let stale_snapshots_dir = snapshot_path.join(".astra-snapshots");
        ...
        && let Err(e) = std::fs::remove_dir(&stale_snapshots_dir)
    {
        tracing::debug!(
            ...
            error = %e,
            "failed to remove stale .astra-snapshots placeholder from snapshot"
        );
    }
...
/// root mount), snapshots go under `.astra-snapshots/` to stay hidden from git;
...
pub fn snapshot_dest_path(btrfs_mount: &Path, subvolume_root: &Path, dest: &Path) -> PathBuf {
    let subdir = if btrfs_mount == subvolume_root {
        BTRFS_SNAPSHOT_SUBDIRS[1] // ".astra-snapshots"
    } else {
...
fn is_safe_snapshot_delete_target_in(snapshot_path: &Path, btrfs_mounts: &[PathBuf]) -> bool {
    ...
    // Parent must be a snapshot-storage dir (`worktrees` / `.astra-snapshots`)...
...
/// `".astra-snapshots"` is used when the btrfs mount IS the repo subvolume
/// (dot-prefixed to stay hidden from git).
pub const BTRFS_SNAPSHOT_SUBDIRS: &[&str] = &["worktrees", ".astra-snapshots"];
...
mod tests {
    ...
    fn test_btrfs_snapshot_metadata_round_trip() {
        let meta = BtrfsSnapshotMetadata {
            ...
            mount_target: PathBuf::from("/home/user/.astra/worktrees/repo/session/wt-abc"),
    ...
    fn test_write_and_remove_btrfs_metadata() {
        ...
        let mount_target = Path::new("/home/user/.astra/worktrees/wt-abc");
    ...
    fn test_snapshot_dest_path_separate_root_mount() {
        ...
        let dest = Path::new("/home/user/.astra/worktrees/repo/session/wt-abc");
    ...
    fn test_snapshot_dest_path_subvol_mount() {
        // btrfs mount IS the subvolume root → snapshots under .astra-snapshots/.
        let mount = Path::new("/workspace/repo");
        let dest = Path::new("/home/user/.astra/worktrees/repo/session/wt-xyz");
        let got = snapshot_dest_path(mount, mount, dest);
        assert_eq!(
            got.parent().unwrap(),
            Path::new("/workspace/repo/.astra-snapshots")
        );
    ...
    fn test_snapshot_dest_path_disambiguates_same_basename_across_repos() {
        ...
        let btrfs_mount = Path::new("/mnt/btrfs");
        let subvolume_root = Path::new("/workspace/repo");
        let dest_a = Path::new("/home/user/.astra/worktrees/repo-a/session/wt-abc");
        let dest_b = Path::new("/home/user/.astra/worktrees/repo-b/session/wt-abc");
    ...
    fn test_snapshot_meta_targets_matches_only_same_dest() {
        ...
        let dest = Path::new("/home/user/.astra/worktrees/repo-a/session/wt-abc");
        ...
        let other = Path::new("/home/user/.astra/worktrees/repo-b/session/wt-abc");
