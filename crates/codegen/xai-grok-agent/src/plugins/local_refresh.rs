...
mod tests {
    ...
    fn refresh_local_install_picks_up_new_agent() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".astra").join("installed-plugins"));
        ...
        let trust = TrustStore::load_from(home.join(".astra").join("trusted-plugins"));
    ...
    fn refresh_preserves_install_subdir_scope() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".astra/installed-plugins"));
    ...
    fn refresh_keeps_stale_when_legacy_subdir_scope_lost() {
        ...
        let install_dir = home.join(".astra").join("installed-plugins");
