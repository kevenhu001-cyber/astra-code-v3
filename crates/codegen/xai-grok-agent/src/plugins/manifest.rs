...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".astra-plugin")).unwrap();

        // Write manifest in .astra-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".astra-plugin/plugin.json"),
