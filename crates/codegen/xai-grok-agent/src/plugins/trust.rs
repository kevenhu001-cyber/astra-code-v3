...
//!
//! Plugins from project directories (`.astra/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.astra/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.astra/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.astra/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_grok_home() so a project's `.astra/trusted-plugins` is never
        // read as the user trust store when neither ASTRA_HOME nor a home dir resolves.
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.astra/trusted-plugins`.
    ...
    /// rewrites `~/.astra/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.astra/trusted-plugins`.
