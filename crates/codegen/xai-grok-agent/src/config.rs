//! Agent definition types — parsed from `.astra/agents/*.md` files.
...
/// Astra + plan mode toolset WITHOUT subagent tools.
///
...
/// Default Astra toolset + `ask_user_question`.
///
...
/// Portable agent identity — parsed from .astra/agents/*.md.
...
pub enum AgentScope {
    /// .astra/agents/ (project-level, highest priority)
    Project,
    /// ~/.astra/agents/ (user-level)
    User,
    /// ~/.astra/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
...
pub enum MemoryScope {
    /// `~/.astra/agent-memory/<name>/`
    User,
    /// `<project>/.astra/agent-memory/<name>/`
    Project,
    /// `<project>/.astra/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            ...
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".astra/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".astra/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        if path_str.contains(".astra/agents/") || path_str.contains(".grok\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".astra/bundled/agents/")
...
impl AgentDefinition {
    ...
    pub fn grok_build_concise() -> Self {
        Self {
            ...
            ..Self::base(
                ...
                "Astra agent with concise output format.",
            )
        }
    }
    /// Astra agent with plan mode tools.
    pub fn grok_build_plan() -> Self {
        Self {
            tool_config: grok_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::GrokBuildPlan,
                "Astra agent with plan mode support.",
            )
        }
    }
    /// Astra + plan mode WITHOUT subagent tools.
    pub fn grok_build_plan_no_subagents() -> Self {
        Self {
            ...
            ..Self::base(
                ...
                "Astra agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Astra agent with the `ask_user_question` tool.
    pub fn grok_build_ask_user() -> Self {
        Self {
            tool_config: grok_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::GrokBuildAskUser,
                "Astra agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    /// Astra Orchestrator — GBL model with full GrokBuild tools
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.astra/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.astra/agent-memory-local/a")
        );
