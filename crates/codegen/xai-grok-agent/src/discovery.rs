...
//!
//! Searches `.astra/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.astra/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.astra/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".astra/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.astra/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.astra/agents` / `.claude/agents`) under each
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.astra/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.astra/agents/` walking from `cwd` up to repo root
/// 2. `~/.astra/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.astra/bundled/agents/` (bundled, lowest priority)
///
...
/// (ASTRA_HOME-aware) plus the legacy literal `~/.astra` when ASTRA_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    grok_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.astra, included only when it differs from grok_home
    // (i.e. ASTRA_HOME points elsewhere) so agents left in the old location are
...
///
/// Project-level `.astra/agents/` has highest priority, then falls back
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Astra
...
/// Load project agent definitions from every `.astra/agents` / `.claude/agents`
...
mod tests {
    ...
    fn user_agent_dirs_dedups_legacy_when_grok_home_is_dot_grok() {
        ...
        assert_eq!(
            count, 1,
            "no duplicate ~/.astra/agents when grok_home == ~/.astra"
        );
    }
    ...
    fn test_discover_finds_md_files() {
        ...
        let agents_dir = tmp.path().join(".astra").join("agents");
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".astra").join("agents");
        let agents_dir_2 = inner_dir.join(".astra").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".astra").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".astra")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".astra")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".astra").join("agents");
        let bundled_dir = home.join(".astra").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".astra")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".astra")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".astra").join("agents");
        let bundled_dir = home.join(".astra").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .astra/agents/ directory — should fall back to built-in
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.astra/agents/) agent named "explore" should NOT shadow
