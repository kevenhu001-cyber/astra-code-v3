...
pub struct PromptContext {
    ...
    /// Legacy model identity label (formerly rendered as `You are <label>`).
    /// Retained for config compatibility; no longer rendered into the prompt.
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default system-prompt identity label.
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "Astra Code";
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "Astra Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "Astra Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.astra/worktrees/my-project/ab-123-a-overlay";
        let ctx = PromptContext {
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a Astra agent"),
