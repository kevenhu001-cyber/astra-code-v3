...
pub enum UserMessageTemplate {
    /// Legacy Astra prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .astra/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// (`~/.astra/projects/<encoded-cwd>/mcps`). Surfaced in
...
mod tests {
    ...
    fn format_rules_section_workspace_then_user() {
        ...
        let user = [
            ...
            RuleEntry {
                path: "/home/dev/.astra/AGENTS.md".into(),
