...
///
/// Priority order: Local (cwd/.astra/skills, cwd/.agents/skills, cwd/.claude/skills) → Intermediate dirs →
/// Repo (repo_root/.astra/skills, repo_root/.agents/skills, repo_root/.claude/skills) → User (~/.astra/skills, ~/.agents/skills, ~/.claude/skills)
/// → additional paths from `config.paths`
/// → Server (injected `config.server_skill_dirs`)
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.astra/bundled`; lowest precedence).
///
...
/// (e.g. `Repo` for `.astra/plugins/`). Native skills always win bare-name
...
pub fn collect_skill_config_dirs(
    ...
    // this list equals the historical `[".astra", ".agents", ".claude", ".cursor"]`.
...
mod tests {
    ...
    async fn server_skills_discovered_and_shadowed_by_local() {
        ...
        write_skill_md(&cwd.path().join(".astra").join("skills").join("dup"), "dup");
    ...
    async fn list_skills_includes_workspace_user_skills() {
        ...
        write_skill_md(
            &user_dir.join(".astra").join("skills").join("my-tool"),
            "my-tool",
        );
    ...
    async fn list_skills_workspace_user_dedup_when_cwd_inside_user_dir() {
        ...
        let user_dir = repo_root.join("x").join("testuser");
        write_skill_md(
            &user_dir.join(".astra").join("skills").join("dedup-skill"),
            "dedup-skill",
        );
    ...
    async fn list_skills_no_workspace_user_dir_no_extra_skills() {
        ...
        write_skill_md(
            &user_dir.join(".astra").join("skills").join("ghost-skill"),
            "ghost-skill",
        );
    ...
    async fn list_skills_workspace_user_dir_with_recursive_skills() {
        ...
        let skills_base = user_dir.join(".astra").join("skills");
    ...
    async fn list_skills_deduplicates_auto_and_config_overlap() {
        ...
        let auto_dir = repo_root.join(".astra").join("skills").join("dup-skill");
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        let auto_dir = repo_root.join(".astra").join("skills").join("overlap-skill");
    ...
    async fn list_skills_ignore_allows_lower_priority_same_name_fallback() {
        ...
        write_skill_md(&cwd.join(".astra").join("skills").join("same"), "same");
        let repo_skill_dir = repo_root.join(".astra").join("skills").join("same");
    ...
    async fn disabled_config_marks_skill_enabled_false() {
        ...
        write_skill_md(
            &repo_root.join(".astra").join("skills").join("commit"),
            "commit",
        );
        write_skill_md(
            &repo_root.join(".astra").join("skills").join("review"),
            "review",
        );
    ...
    async fn disabled_config_empty_leaves_all_enabled() {
        ...
        write_skill_md(
            &repo_root.join(".astra").join("skills").join("deploy"),
            "deploy",
        );
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        for name in [".astra", ".agents", ".claude", ".cursor"] {
        ...
        assert!(ends_with(&dirs, ".astra"), "grok must remain: {dirs:?}");
    }
    ...
    fn dedupe_same_scope_cross_harness_loser_resurfaces() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "review",
                "/u/.astra/skills/review/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "review",
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.astra/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.astra/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.astra/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.astra/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.astra/skills/japandi/SKILL.md",
        ...
        assert_eq!(out.len(), 1);
        assert!(out[0].path.contains(".astra"));
    }
    ...
    async fn copied_skill_dir_with_stale_frontmatter_name_surfaces_both() {
        ...
        let skills_dir = repo_root.join(".astra").join("skills");
