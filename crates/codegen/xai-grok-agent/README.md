...
in `.astra/agents/` (project-level) or `~/.astra/agents/` (user-level).
...
let def = AgentDefinition::from_file(".astra/agents/code-reviewer.md")?;
...
// Find all .md files in .astra/agents/ directories
...
1. **Project-level** (highest priority): `.astra/agents/*.md` — walk
   from `cwd` up to the git repository root. Files found closer to
   `cwd` take priority.
2. **User-level**: `~/.astra/agents/*.md`
...
example, a project `.astra/agents/code-reviewer.md` shadows a
