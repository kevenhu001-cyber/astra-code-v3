...
//! elected leader) plus a fixed leader socket beneath its `ASTRA_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `ASTRA_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared ASTRA_HOME => one leader; the socket lives beneath it so
    ...
    /// The cluster's sessions root: `ASTRA_HOME/sessions` (layout below is
