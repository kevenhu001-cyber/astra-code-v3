...
pub struct AcpSessionFs {
    ...
    /// (`~/.astra/worktrees/.../b-overlay/foo.rs`), the adapter rewrites it
...
mod tests {
    ...
    fn resolve_path_rewrites_display_to_overlay() {
        let result = test_resolve(
            "/root/.astra/worktrees/proj/ab-123-b-overlay",
            Some("/testbed/proj"),
            "/testbed/proj/src/main.rs",
        );
        assert_eq!(
            result,
            PathBuf::from("/root/.astra/worktrees/proj/ab-123-b-overlay/src/main.rs")
        );
    }

    #[test]
    fn resolve_path_passes_through_overlay_path() {
        let overlay_path = "/root/.astra/worktrees/proj/ab-123-b-overlay/src/main.rs";
        let result = test_resolve(
            "/root/.astra/worktrees/proj/ab-123-b-overlay",
            Some("/testbed/proj"),
            overlay_path,
