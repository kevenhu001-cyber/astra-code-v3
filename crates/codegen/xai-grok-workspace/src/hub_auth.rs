//! Hub [`AuthProvider`] from `~/.astra/auth.json` for the standalone
...
pub fn default_auth_path() -> anyhow::Result<PathBuf> {
    let grok = xai_grok_config::user_grok_home()
        .ok_or_else(|| anyhow::anyhow!("no user grok home (set $ASTRA_HOME or $HOME)"))?;
...
/// the default credential path (`~/.astra/auth.json`).
pub fn provider(
    hub_url: &Url,
