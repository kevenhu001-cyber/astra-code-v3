...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.astra/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.astra/hooks/`).
    ...
    /// `<cwd>/.astra/lsp.json`: `false` drops them at load, `true` keeps them. The
