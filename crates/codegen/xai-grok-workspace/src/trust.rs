...
//!
//! Persists per-folder trust decisions to `~/.astra/trusted_folders.toml`.
...
//! neither `$ASTRA_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.astra/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.astra/`.
...
/// ROOT (via the `~/.astra/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.astra/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.astra/trusted-hook-projects` file listed one canonical project
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        // how a cloned repo's own `<repo>/.astra/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
    ...
    /// Point `ASTRA_HOME` at an isolated tempdir and register one grok-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `ASTRA_HOME` on
