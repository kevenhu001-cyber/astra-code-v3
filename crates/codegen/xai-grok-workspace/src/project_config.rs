...
//! `.astra/config.toml` files by walking from `cwd` up to the git root.
//!
...
/// True when `config_path` is `$ASTRA_HOME/config.toml` (user tier, not project).
...
/// Find all `.astra/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.astra/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.astra/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.astra/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        .iter()
        .rev()
        .map(|dir| dir.join(".astra").join("config.toml"))
...
mod tests {
    ...
    fn find_project_configs_excludes_user_grok_config_file() {
        ...
        std::fs::write(project.join(".astra/config.toml"), "# project\n").unwrap();
