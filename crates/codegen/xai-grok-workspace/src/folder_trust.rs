...
//! [`crate::trust::TrustStore`] (`~/.astra/trusted_folders.toml`). The
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.astra`).
...
/// `[permission]` / `.claude` / `.astra/agents` / subdir-launch gaps. NOT itself
/// the trust gate.
pub fn repo_config_kinds(cwd: &Path) -> Vec<&'static str> {
    collect_repo_config_kinds(cwd, false)
}

/// Whether a project `.astra/config.toml` `[permission]` value would contribute
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    // Project `.astra/config.toml` declaring repo-controlled code-exec or
    ...
    // Project `.astra/lsp.json`.
    if cwd.join(".astra").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.astra/hooks/evil.json`) would
    ...
    // that must be gated — else a plugin clone (e.g. `.astra/plugins/evil/`, even
    ...
    // Project AGENT dirs (`.astra/agents` / `.claude/agents`): a project agent
    ...
    if directory_present_or_uncertain(&hook_root.join(".astra").join("workflows")) {
        hit!("workflows");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .astra/lsp.json / hooks) \
...
mod tests {
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.astra/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".astra").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_roles() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".astra").join("roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_personas() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".astra").join("personas")).unwrap();
    ...
    fn project_subagent_marker_at_repo_root_is_absent_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".astra/roles")).unwrap();
    ...
    fn repo_configs_present_detects_project_workflows_from_subdir() {
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".astra").join("workflows")).unwrap();
        let subdir = tmp.path().join("crates").join("inner");
        std::fs::create_dir_all(&subdir).unwrap();
        assert!(repo_configs_present(&subdir));
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".astra").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".astra").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.astra/agents` — even when launched from a SUBDIR (the cwd→git-root walk
    ...
    // ASTRA_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("ASTRA_HOME", home.path());
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty ASTRA_HOME (no
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // never prompts). ASTRA_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ASTRA_HOME", home.path());
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty ASTRA_HOME (no config.toml/managed config) +
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); ASTRA_HOME-isolated
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // ASTRA_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("ASTRA_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // ASTRA_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("ASTRA_HOME", home.path());
        let _sim = simulate_release_build();
