    ...
    pub fn description(self) -> Option<&'static str> {
        match self {
            ...
            Self::GrokConfig => Some(
                "Note: This edit contains changes to Astra config, which can alter permissions, tools, and other behavior in later sessions.",
            ),
            Self::GrokSandbox => Some(
                "Note: This edit contains changes to the Astra sandbox config, which can loosen filesystem and network restrictions on commands.",
            ),
...
/// Astra config files that alter permissions (`config.toml`, the
...
/// directly under a custom `$ASTRA_HOME`, which the component match cannot see.
...
fn protected_grok_config_file_with_home(
    ...
    let in_dot_grok = components.len() >= 2 && components[components.len() - 2] == ".astra";
...
fn protected_grok_hook_root(path: &Path, components: &[&str]) -> bool {
    components.windows(2).any(|pair| pair == [".astra", "hooks"])
        || components.ends_with(&[".astra", "hooks-paths"])
...
mod tests {
    ...
    fn sensitive_edit_targets_and_lexical_aliases_prompt() {
        for path in [
            ...
            "/work/project/.astra/sandbox.toml",
        ] {
            assert!(
        ...
        for path in [
            "/work/src/main.rs",
            "/work/project/.astra/config.toml/backup",
    ...
    fn edit_target_protection_classifies_reasons() {
        let cases = [
            ...
            (
                "/home/user/.astra/managed_config.toml",
                ProtectedEditReason::GrokConfig,
            ),
            (
                "/home/user/.astra/requirements.toml",
                ProtectedEditReason::GrokConfig,
            ),
            (
    ...
    fn sensitive_edit_targets_include_hook_roots() {
        for path in [
            "/home/user/.astra/hooks/evil.json",
            "/home/user/.astra/hooks/nested/deep.json",
            "/home/user/.astra/hooks-paths",
            "/home/user/.claude/settings.json",
            "/home/user/.claude/settings.local.json",
            "/home/user/.cursor/hooks.json",
            "/work/project/.astra/hooks/local.json",
            "/work/project/.astra/hooks-paths",
        ] {
            assert!(
                edit_target_protection(Path::new(path)).is_some(),
        ...
        for path in [
            "/home/user/.astra/hooks-disabled/note.json",
            "/home/user/.astra/hooks-evil/note.json",
    ...
    fn sensitive_edit_targets_follow_symlinks() {
        ...
        .unwrap();
        let grok_hook = outside.path().join(".astra/hooks/evil.json");
    ...
    /// A custom `$ASTRA_HOME` has no `.grok` path component, so the live
    ...
    fn grok_config_files_under_custom_grok_home_are_protected() {
        ...
        for (file, reason) in [
            ...
            assert_eq!(
                ...
                Some(reason),
                "{file} directly under $ASTRA_HOME must be protected"
            );
        }
    ...
    /// The resolved-symlink arm of the grok-home match must decide: `$ASTRA_HOME`
