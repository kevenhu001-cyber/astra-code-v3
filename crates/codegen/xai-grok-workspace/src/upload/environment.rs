...
pub struct WorkspaceEnvironment {
    ...
    /// Sandbox profile name (`$ASTRA_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("ASTRA_SANDBOX_PROFILE")
...
mod tests {
    ...
    fn assemble_populates_every_field() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("git@github.com:topodrive-ai/example.git".to_string()),
        );
        ...
        assert_eq!(
            ...
            Some("git@github.com:topodrive-ai/example.git")
        );
    ...
    fn serializes_with_all_fields_present() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("host-1".to_string()),
            Some("/work/repo".to_string()),
            Some("https://github.com/topodrive-ai/example".to_string()),
        );
    ...
    fn git_repo_facts_strips_remote_url_credentials() {
        ...
        assert_eq!(url, "https://github.com/topodrive-ai/example.git");
    }
}
