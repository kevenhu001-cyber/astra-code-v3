...
//! `~/.astra/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
...
/// `~/.astra/memory/`. Workspace-scoped files live under a directory
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.astra/memory/`.
    ///
    ...
    ///
    /// File path: `~/.astra/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    pub fn ensure_initialized(&self) -> std::io::Result<()> {
        ...
        if !global_file.exists() {
            std::fs::write(
                ...
                "# Global Memory\n\
                 \n\
                 > This file is automatically managed by Astra's memory system.\n\
