// claude_import.rs
// Scans Claude settings and generates TOML patches for .astra/config.toml.
//
...
pub enum ImportScope {
    /// User-level: writes to `~/.astra/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.astra/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.astra/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.astra/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.astra/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.astra/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.astra/config.toml`.
///
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.astra/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.astra/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        ...
        // Hooks are written separately to ~/.astra/hooks/imported-from-claude.json.
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".astra").join("config.toml");
        ...
        let hooks_dir = project_root.join(".astra").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .astra/hooks/ JSON files in apply_hooks_to_dir,
...
/// (Claude-compatible). The native hooks loader scans `.astra/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .astra/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.astra) don't interfere.
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `ASTRA_HOME` plumbing which is intentionally out of scope.
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.astra/config.toml` may legitimately have the marker
