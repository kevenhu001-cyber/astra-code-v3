...
/// hot-reload watcher when `<cwd>/.astra/config.toml`,
...
/// Hot-reload the model catalog from `~/.astra/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .astra/plugins under it, plus
