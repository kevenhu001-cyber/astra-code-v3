...
mod tests {
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".astra/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".astra/roles")).unwrap();
        std::fs::write(
            project_root.join(".astra/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".astra/roles/reviewer.toml"),
