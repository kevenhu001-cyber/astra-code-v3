...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.astra/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.astra/config.toml`,
    ...
    /// (`<cwd>/.astra/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ProjectMcpServersChanged {
        /// The project root whose `.astra/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.astra/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub(crate) async fn run(
        ...
        loop {
            ...
            // same tick (e.g. `~/.astra/config.toml` AND
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.astra/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.astra/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.astra/config.toml` | `<cwd>`           |
...
/// for `.astra/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.astra/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
mod tests {
    ...
    /// The hash must reflect ancestor `.astra/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".astra").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .astra/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.astra/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.astra/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.astra/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.astra/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.astra/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.astra/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
