...
/// A feedback entry persisted to `~/.astra/sessions/.../feedback.jsonl`.
///
...
/// Searches across ALL cwd directories under `~/.astra/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.astra/sessions/`.
///
...
/// Walk `~/.astra/sessions/` and delete files with mtime older than `ttl_days`.
