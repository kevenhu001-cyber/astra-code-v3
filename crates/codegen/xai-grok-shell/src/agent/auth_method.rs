...
pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `grok login`, set XAI_API_KEY, or add api_key to ~/.astra/config.toml.";
...
pub(crate) fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.astra/auth.json".to_string())),
    )
}
...
pub(crate) fn grok_com_auth_method(
    ...
    let name = label.unwrap_or("Astra");
...
mod tests {
    ...
    /// `~/.astra/config.toml` skeleton from the bug report, walks it through
