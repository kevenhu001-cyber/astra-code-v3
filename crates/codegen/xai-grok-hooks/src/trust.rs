...
// (`~/.astra/trusted_folders.toml`) is the single authority for whether a repo's
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user grok home (set $ASTRA_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$ASTRA_HOME/disabled-hooks`, or `None` when no user grok
/// home resolves.
