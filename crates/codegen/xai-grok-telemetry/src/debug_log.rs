...
//!   firehose to `~/.astra/debug/<session_id>.txt` (one file per session), with a
...
pub(crate) enum DebugTarget {
    /// `GROK_DEBUG_LOG=1` → route per session into `<dir>` (`~/.astra/debug`).
...
/// per session into `~/.astra/debug`, an explicit path writes a single file.
///
...
/// `~/.astra/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
