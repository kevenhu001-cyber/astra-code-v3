...
/// `~/.astra/logs/unified.jsonl` (those bursts inflate exactly the counters
...
/// healing a stale handle rather than re-resolving `$ASTRA_HOME` — which also
...
mod tests {
    ...
    /// writer onto the developer's real `~/.astra/logs/unified.jsonl`.
    #[ctor::ctor]
