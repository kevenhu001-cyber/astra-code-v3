...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$ASTRA_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `grok_home()` OnceLock). A seeded `$ASTRA_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("ASTRA_HOME")
    ...
    /// `$ASTRA_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`ASTRA_HOME` /
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$ASTRA_HOME` even when offline mode was not
...
mod tests {
    ...
    /// `$ASTRA_HOME` env so tests never race the parallel harness.
