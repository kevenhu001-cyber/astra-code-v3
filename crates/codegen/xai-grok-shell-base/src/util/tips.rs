...
//! persisted to `~/.astra/tip_cursor.json`.
...
/// Load the cursor from `~/.astra/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.astra/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.astra/tip_cursor.json`, so every session sees the next tip
