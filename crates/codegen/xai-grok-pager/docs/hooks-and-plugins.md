...
Astra supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.astra/hooks/`
- **Project hooks** — from `.astra/hooks/` in your repo
...
Plugin hooks automatically receive `ASTRA_PLUGIN_ROOT` and `ASTRA_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.astra/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `astra plugin marketplace add <source>`)
...
Sources land in `~/.astra/config.toml`:
...
Or in `~/.astra/settings.json` / `~/.claude/settings.json`:
