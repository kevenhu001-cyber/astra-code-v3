...
Hooks let you run a script or send an HTTP request at key moments in a Astra session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Astra calls when a specific lifecycle event occurs. Hooks can:
...
2. Create a hook file, e.g. `~/.astra/hooks/session-start.json`:
...
|-------|------|----------|-------|
| Global | `~/.astra/hooks/*.json` | Always | Personal hooks |
...
| Config | `~/.astra/config.toml` | Always | Your hooks alongside the rest of your config |
| Config | `managed_config.toml` (`$ASTRA_HOME` and `/etc/astra`) | Always | Organization-distributed hooks (server-synced and on-device) |
...
Config-file hooks live in the same TOML your organization already controls; see [Hooks in Config Files](#hooks-in-config-files) for the format. The compatible vendor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.astra/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run; until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.astra/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.astra/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`ASTRA_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Astra accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Astra skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Astra maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
When an event fires, Astra resolves it in four steps:
...
Hooks can also live directly in your Astra config, so a team can distribute them with the rest of their configuration instead of shipping separate JSON files. The same `hooks` object is read from three TOML files:

| File | Tier | Who sets it |
|------|------|-------------|
| `~/.astra/config.toml` | User | You |
| `managed_config.toml` (`$ASTRA_HOME`, `/etc/astra`) | Managed / system | Your organization |
...
A script that tracks busy and idle should key on `promptId`, following the rules below. astra mints one per turn, but a client that supplies its own in `_meta` owns its uniqueness, so treat the id as opaque and scope it to the session.
...
- **Settle unconditionally when there is no `promptId`.** That is astra reporting on the session
...
- **StopFailure classes**: astra emits six (`rate_limit`, `authentication_failed`, `invalid_request`, `server_error`, `max_output_tokens`, `unknown`). Capacity errors (503/529) classify as `rate_limit`. A matcher on an error class astra does not emit never fires.
- **Default timeout**: astra defaults observe hooks to 5 seconds, which is shorter than most. Set `timeout` explicitly on an imported hook that does real work.
- **`UserPromptSubmit` is observe-only**: astra ignores its exit code and its stdout, so an imported prompt-validation hook silently stops blocking. Use `PreToolUse` to enforce.
- **`StopCancelled` is grok-specific**: a config that uses it is not portable to a runtime with no interrupt hook.
- **`idle_prompt` fires on any turn end**: astra fires it after an interrupted or errored turn too, not only a completed one, because it reports a state rather than an outcome. Its `message` is display text and can change between releases, so match on `notificationType` instead.
- **Subagent identity is `subagentType`, not `agent_type`**: astra puts it in the payload of the events that can fire inside a subagent, matching its own `SubagentStart`/`SubagentStop`, rather than in the common fields.
- **permission_mode values**: astra emits `default`, `auto`, `plan`, or `bypassPermissions`. Claude's `acceptEdits`/`dontAsk` have no astra equivalent (astra's `auto` is the nearest), so a check like `permission_mode === "acceptEdits"` never matches.
...
- **`/goal`**: astra's goal loop is a separate feature that runs before the stop gate; it is not a prompt-type Stop hook.
...
Astra sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `ASTRA_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `ASTRA_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `ASTRA_SESSION_ID`     | The unique identifier of the current Astra session. |
| `ASTRA_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `ASTRA_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Astra additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `ASTRA_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `ASTRA_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`ASTRA_PLUGIN_ROOT`, `ASTRA_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
Press `r` in the Hooks tab to reload all hooks from disk. Astra re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.astra/hooks/`) run with your user permissions; treat them like shell scripts.
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug ASTRA_LOG_FILE=/tmp/grok.log astra`, then check `/tmp/grok.log`.
