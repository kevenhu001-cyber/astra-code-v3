...
- **`astra dashboard`** — launch the TUI into the dashboard.
- **`/dashboard`** (aliases **`/agents-dashboard`**, **`/sessions`**) — open
  from inside a session.
- **`Ctrl+\`** — same view as the slash command.

Hidden in minimal mode. Set `ASTRA_AGENT_DASHBOARD=0` or
...
```
 Astra · Dashboard — 4 agents · 2 awaiting
...
State icons match other session lists in Astra:
...
Per-user preferences under `[dashboard]` in `~/.astra/config.toml`:
