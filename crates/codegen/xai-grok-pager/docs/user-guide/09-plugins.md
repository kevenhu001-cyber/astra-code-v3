...
A plugin bundles skills, slash commands, agents, hooks, and MCP servers into one installable unit. You get plugins from a marketplace, install the ones you want, and Astra loads what they add. To build and share your own, see [Create your own marketplace](#create-your-own-marketplace).
...
1. **Add the marketplace** so Astra can show what it offers. Nothing installs yet.
...
```bash
astra plugin marketplace add my-org/team-plugins                  # GitHub shorthand (owner/repo)
astra plugin marketplace add https://gitlab.com/acme/plugins.git  # any git host, include https:// and .git
astra plugin marketplace add ./my-marketplace                     # a local folder
```

List, refresh, and remove sources with `astra plugin marketplace list`, `astra plugin marketplace update [<name>]`, and `astra plugin marketplace remove <url>`.
...
Place this file at `~/.astra/settings.json` or `~/.claude/settings.json`.
...
```bash
astra plugin install deploy-tools --trust
```
...
Run `astra plugin install <source>` without `--trust` and Astra shows the source, warns that installing activates the plugin's hooks, MCP servers, and skills, then stops. Add `--trust` to go ahead. Only install plugins from sources you trust (see [Trust and security](#trust-and-security)).

A plugin's skills appear in the slash menu. When a skill name is ambiguous, Astra shows the qualified form prefixed by the plugin name, for example `/deploy-tools:release`. To pick up a newly installed plugin, press `r` in the Plugins tab or start a new session.
...
```bash
astra plugin list [--json] [--available]   # installed plugins (--available requires --json)
astra plugin uninstall <name> [--confirm] [--keep-data]   # aliases: rm, remove
astra plugin update [<name>]               # omit the name to update every plugin
astra plugin enable <name>
astra plugin disable <name>
astra plugin details <name>                # show the plugin's component inventory
```
...
Set these in `~/.astra/config.toml`:
...
Plugins are off by default, so list one in `enabled` to turn it on, or in `disabled` to discover it but skip loading it. Each entry is a plain plugin name (from `astra plugin list`) or a full ID (`<scope>/<hash>/<name>`).

To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.astra/pager.toml`.
...
3. **List the plugins in `.astra-plugin/marketplace.json`.** This is the index Astra reads.
...
my-org-plugins/
  .astra-plugin/
    marketplace.json      # the index Astra reads (required)
...
Astra reads the index from `.astra-plugin/marketplace.json`. It also accepts `.astra-plugin/plugin.json` and the `.claude-plugin/` equivalents.
...
Validate a plugin before publishing with `astra plugin validate [<path>]`, and tag a release from the manifest version with `astra plugin tag [<path>] [--push]`. Then point people at the repository. They add it once and install the plugins they want:

```bash
astra plugin marketplace add my-org/my-org-plugins   # GitHub shorthand, a git URL, or a local path
astra plugin install gdrive --trust
```
...
# Plugins stay off until enabled. List plugin names (from `astra plugin list`)
# or full IDs (`<scope>/<hash>/<name>`).
[plugins]
enabled = ["gdrive"]
```

For a hands-off install with no per-person step, also place the plugin's files where Astra discovers and trusts them automatically: `~/.astra/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
You can also set `ASTRA_MARKETPLACE_REQUIRE_SHA=1`. Both only tighten the policy; neither turns it back off. Publish `sha` values in your marketplace's `plugin-index.json` so installs from it satisfy the rule. Plugins vendored directly inside a marketplace repository are copied from that repository's checkout, so pin them the same way, with `sha` values in `plugin-index.json`.
...
Marketplaces distribute Astra content: skills, commands, agents, hooks, and MCP server configurations. They do not install a program onto a machine. A skill or MCP server that runs a helper binary (for example a custom sign-in tool) still needs that binary delivered separately, bundled with your deployment or pushed through your device-management tool.

---

## Troubleshooting

**A plugin you installed isn't showing up.** Plugins are off until enabled. Check `astra plugin list`, then add the plugin's name or ID to `[plugins].enabled`, or press `Space` on it in the Plugins tab. Reload with `r` in the Plugins tab or start a new session.

**A plugin's hooks or MCP servers don't run.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.astra/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).

**A skill or MCP server from a marketplace is missing.** Refresh the source with `astra plugin marketplace update`, confirm the plugin is installed and enabled, and, if your organization restricts sources, check that the marketplace is still allowed (see [Distribute across an organization](#distribute-across-an-organization)). Some MCP servers require a sign-in and will not appear until you authenticate.
...
**See exactly what loaded.** Run `astra inspect` (add `--json` for machine-readable output) to list every discovered plugin and the skills, agents, hooks, and MCP servers it provides, each labeled with its `plugin: <name>` source.
...
An optional `plugin.json` manifest can override paths or add metadata; without one, Astra discovers components from these standard directories. For example, a `team-tools` plugin might bundle a deploy skill, a code-review agent, pre-commit hooks, and a Linear MCP server, installed together in one step.
...
|----------|-------------|
| `ASTRA_PLUGIN_ROOT` | Absolute path to the plugin's installed directory. |
| `ASTRA_PLUGIN_DATA` | Absolute path to the plugin's writable data directory, for state, caches, and logs. |

Astra sets these and overrides any same-named value in the hook's `env` map (the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases are set too). See the [Hooks guide](10-hooks.md) for every variable passed to hooks.
