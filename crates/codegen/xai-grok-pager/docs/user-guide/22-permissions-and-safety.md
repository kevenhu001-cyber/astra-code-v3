...
Control what Astra can access and do: permission modes, allow/ask/deny rules, hooks, and the optional OS-level sandbox.

- **Modes** set how often Astra asks for approval (always-approve, auto, ask, and related).
...
When Astra edits a file, runs a command, or calls an external tool, it may pause for approval. Permission modes control how often that happens.
...
```bash
astra -p "Run the tests" --always-approve
...
```bash
astra --always-approve -p "Run the test suite"
astra --permission-mode auto
astra agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
astra -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
...
Organizations can prevent always-approve from being enabled via CLI, TUI, or `/always-approve`. Set this in `requirements.toml` (user-level under `~/.astra/`, or system-wide under `/etc/astra/` for enforcement users cannot remove):
...
Astra can still load Claude-style permission **rules** from managed settings; always-approve is locked with `requirements.toml` as shown above.
...
Astra reads permission rules from three compatible sources. Rules from all sources are merged into one set; a rule's effect depends on its action (`deny` > `ask` > `allow`), not on which file it came from.
...
|-------|------|-----------------------|
| Global (all projects) | `~/.astra/config.toml` | No |
| Project (committed) | `<project>/.astra/config.toml` | Yes (commit it) |
| Project (personal) | `<project>/.claude/settings.local.json` | No (gitignore it) |
| Interactive grants | Stored internally by Astra, per project | No |

Notes on scoping:

- Astra discovers a `.astra/config.toml` at every directory level from the repository root down to your working directory, so a subdirectory can add rules on top of the repo root's.
...
- Astra has no native `config.local.toml`. For personal, uncommitted rules in a project, use `.claude/settings.local.json`; Astra reads it directly (see [Claude Code Compatibility](#3-claude-code-compatibility-claudesettingsjson)).
...
To stop prompts for a specific command in one project, add a narrow allow rule to that project's `.astra/config.toml` (or `.claude/settings.json`):
...
```bash
astra -p "Review the API changes" \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)' \
  --allow 'Read' \
  --allow 'Grep' \
...
### 2. Native Configuration (`~/.astra/config.toml` and `.astra/config.toml`)
...
Rules from the global `~/.astra/config.toml` and every project `.astra/config.toml` (from the repo root down to your working directory) are merged into one rule set, alongside any `.claude/settings.json` rules.

Managed configuration deployed by your organization also contributes `[permission]` rules: the system `/etc/astra/managed_config.toml`, and a user-level copy that Astra maintains automatically at `~/.astra/managed_config.toml`. Managed rules merge like rules from any other source, with two properties specific to managed `allow` rules: your own `deny` and `ask` rules win over a managed `allow` (severity ordering), and a catch-all managed `allow` is ignored when always-approve is locked off. For rules that users cannot edit away, use the root-owned system `/etc/astra/requirements.toml`.
...
Astra reads `~/.claude/settings.json` and `~/.claude/settings.local.json`, plus the project-level `<project>/.claude/settings.json` and `settings.local.json` (walking up to the repo root). The native `.astra` source for permission rules is `config.toml`, described in the section above.
...
Supported `defaultMode` values include `default`, `auto`, `acceptEdits`, `bypassPermissions`, `dontAsk`, and `plan`. Astra reads `defaultMode` from its canonical location under `permissions`; a top-level `defaultMode` is also accepted when the nested key is absent.
...
**Chained commands.** Astra parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full Astra tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. Astra tool names carry no `mcp__` prefix.
...
Interactive grants are stored in Astra's own state directory under your home directory, scoped to the git repository you launched Astra in (its repository root), so a grant accepted at the repo root also applies in sessions started from a subdirectory of the same repository. Outside a git repository, grants are scoped to the launch directory, and each git worktree keeps its own grants. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.astra/config.toml` instead.
...
```bash
chmod +x ~/.astra/hooks/git-gh-only.sh
```
...
```bash
astra -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
...
3. **Review project configuration from unfamiliar sources.** Project permission rules in `.astra/config.toml` and `.claude/settings.json`, including `allow` rules, apply without a separate trust prompt. Review them, and any project hooks, before working in an unfamiliar checkout (see the security notes in [10-hooks.md](10-hooks.md)).
