...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.astra/agents/` or `~/.astra/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.astra/personas/`; applied during subagent resolution |
...
```bash
export ASTRA_SUBAGENTS=0              # Environment variable
```
...
Astra discovers file-based personas from these locations, in priority order:

- `.astra/personas/*.toml` (project)
- `~/.astra/personas/*.toml` (user)
...
> **Note:** Astra applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Astra resolves the effective model and reasoning effort in this order, highest priority first:
...
Astra manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
# instructions_file = ".astra/personas/concise.md"  # or load from a file
```

Astra also discovers roles from `.astra/roles/*.toml` and personas from `.astra/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Astra shows running and finished work in side panes on the agent screen:
