...
Astra reads settings from config files, environment variables, and CLI flags. This page covers the common options.
...
2. **Environment variables** (e.g. `XAI_API_KEY`, `ASTRA_MEMORY`)
3. **`requirements.toml` / MDM** (org-enforced; clamps every config layer below, including the overlay)
4. **`ASTRA_CONFIG` / `ASTRA_CONFIG_PATH` overlay** (above `config.toml` and managed, below `requirements.toml` / MDM)
5. **config.toml** (`~/.astra/config.toml`)
6. **`managed_config.toml`** (org-deployed defaults; below `config.toml`)
7. **Built-in defaults**

Within the config-file tier, the layers merge lowest-to-highest: `managed_config.toml` → `config.toml` → `ASTRA_CONFIG` overlay → `requirements.toml` / MDM. So `requirements.toml` and MDM clamp **both** your `config.toml` and the overlay.

`ASTRA_CONFIG` / `ASTRA_CONFIG_PATH` (tier 4) are config **overlays**: a merged config layer, not direct-setting environment variables like `XAI_API_KEY` (tier 2). They set config keys (subject to the allowlist below), so read them as part of the config-file tier rather than the env-var tier.

### Injecting config with `ASTRA_CONFIG`

A harness or ACP client that launches `astra agent stdio` can inject settings without writing a `config.toml` or relocating `$ASTRA_HOME`:

- **`ASTRA_CONFIG`**: an inline JSON object overlay.
- **`ASTRA_CONFIG_PATH`**: an *additional* file overlay (not a replacement for `config.toml`), a JSON or TOML file read by its extension (`.json` → JSON, else TOML). `ASTRA_CONFIG` wins if both are set. An empty `ASTRA_CONFIG` is treated as unset, and a malformed one logs a warning and falls through to `ASTRA_CONFIG_PATH`.

The overlay is **deep-merged** on top of your `config.toml` (it overrides only the keys it sets), placed above the user/managed layers but **below** `requirements.toml` / MDM so an enterprise pin still wins. A malformed blob is ignored with a warning. This mirrors `CODEX_CONFIG` from the `codex-acp` adapter (a JSON object merged into the session config); Astra is ACP-native, so the overlay lives in the agent itself. It only affects settings read from the merged config, and it is **not** a permission-escalation path. The overlay is confined, fail-closed, to an **allowlist** of soft settings (`models`, `features`, a narrowed `toolset`, and a `shell_environment_policy` limited to its filter fields, which select among env names the launcher already controls and cannot inject an env value into tool subprocesses); every other table is dropped at the choke point, so the overlay cannot spawn commands, set auth policy, redirect network traffic, elevate trust, or add a discovery source. Even on the allowlisted settings, a specific set of security gates read the raw disk layers rather than the overlay. The `ConfigLayers::env_overlay` rustdoc is the canonical list of what the overlay can and cannot reach and which gates read it overlay-free; see also the [internal environment-variables reference](../internal/22-environment-variables.md). Use `ASTRA_DEFAULT_SELECTED_PERMISSION` for headless permission control. For example, to set the default reasoning effort:

```bash
ASTRA_CONFIG='{"models": {"default_reasoning_effort": "high"}}' astra agent stdio
```
...
Location: `~/.astra/config.toml`. If the file is missing, Astra uses its built-in defaults, so you only need to set the values you want to override.
...
# Env vars ASTRA_MAX_PARALLEL_IMAGE_GEN_CALLS / ASTRA_MAX_PARALLEL_VIDEO_GEN_CALLS
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); Astra writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
You can also override this with `ASTRA_DEFAULT_SELECTED_PERMISSION`, which is handy for headless or agent test runs that shouldn't mutate `config.toml`. Precedence: env var → `config.toml` → `always_allow_all_sessions`.
...
`[ui] screen_mode` is the **default render mode** for plain `astra` launches. Set it from `/settings` → **Default screen mode** (restart required) or edit `config.toml` by hand — both write the file. CLI flags (`--minimal` / `--fullscreen`) and slash commands (`/minimal` / `/fullscreen`) are session-scoped and do **not** write this key; after a slash switch, the reverse command returns you for that session only.
...
`allow_local` is off by default (SSRF fail-closed). Turn it on (or set `ASTRA_WEB_FETCH_ALLOW_LOCAL=1`) and `web_fetch` may reach **explicit** loopback hosts only — private, link-local, and cloud-metadata ranges stay blocked. Resolution: TOML > env > default off.
...
`[toolset.ask_user_question]` is honored across **requirements.toml**, **managed config**, and your user **`config.toml`**. Precedence: requirements → env (`ASTRA_ASK_USER_QUESTION_TIMEOUT_ENABLED` / `ASTRA_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed → defaults. Set `timeout_enabled = false` in your user config to disable the automatic questionnaire timeout for yourself; `timeout_secs` must be a positive integer. You can also toggle `timeout_enabled` from `/settings` → **Ask-Question timeout** (under Agent & Approval); changes apply to newly started sessions.
...
MCP servers can also be set per-project in `.astra/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.astra/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.astra/config.toml` (current dir) > `<repo-root>/.astra/config.toml` > `~/.astra/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions. Enable memory with `ASTRA_MEMORY=1`, `[memory] enabled = true`, or managed remote settings.
...
Background workflows — the `workflow` tool, named `.astra/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.

```toml
[workflows]
enabled = false                       # disable background workflows (or ASTRA_WORKFLOWS=0)
```
...
`astra inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences: remembered answers and modal layout. Astra writes these for you as you use the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** these to your user `~/.astra/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "astra"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `astra`. |
...
| Astra Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, Astra detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.

#### Notification hooks

Run your own commands when events fire. Hooks receive `$ASTRA_EVENT`, `$ASTRA_MESSAGE`, and `$ASTRA_SESSION_ID` in the environment.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Astra' -message '$ASTRA_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$ASTRA_MESSAGE' ntfy.sh/my-grok-alerts"
...
- **`[features] telemetry`** / `ASTRA_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **Coding data, retention, and training** — the Settings row `/privacy` opens; coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `ASTRA_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `ASTRA_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by Astra — leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= ASTRA_EXTERNAL_OTEL)
...
- `minimum_version` (`ASTRA_MINIMUM_VERSION`) is a soft anti-downgrade floor. The
  updater skips a target below it and keeps the current version. It never blocks
  startup.
- `maximum_version` (`ASTRA_MAXIMUM_VERSION`) is a soft ceiling. The updater caps
  its target at it and never installs above it.
- `required_minimum_version` (`ASTRA_REQUIRED_MINIMUM_VERSION`) and
  `required_maximum_version` (`ASTRA_REQUIRED_MAXIMUM_VERSION`) are hard bounds. If
  the running version is outside the range, the CLI exits at startup and instructs
  the user to install an approved version. `astra update` and `astra --version` keep
...
- An explicit `astra update --version X` is allowed above the ceiling, to recover
...
Location: `~/.astra/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| `ASTRA_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `ASTRA_OIDC_ISSUER` | OIDC issuer URL |
| `ASTRA_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `ASTRA_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |

### Features

| Variable | Description |
|----------|-------------|
| `ASTRA_MEMORY` | Enable (`1`) or disable (`0`) cross-session memory |
| `ASTRA_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `ASTRA_WORKFLOWS` | Enable (`1`) or disable (`0`) background workflows and select the `/goal` driver (default on: host-owned workflow driver; off: legacy `update_goal`) |
| `ASTRA_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `ASTRA_WEB_FETCH_ALLOW_LOCAL` | Allow `web_fetch` to explicit loopback hosts only (`localhost` / `127.0.0.0/8` / `::1`). Same as `[toolset.web_fetch] allow_local`. Default off; private/metadata stay blocked. |
| `ASTRA_AGENT` | Custom agent definition path or name |
| `ASTRA_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |
| `ASTRA_EXIT_TIMEOUT_SECS` | Seconds after a quit is requested before the process is force-exited if teardown hangs (default: 20, `0` disables; a hard exit follows 5s later) |

### Logging

| Variable | Description |
|----------|-------------|
| `ASTRA_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `ASTRA_LOG_FILE` log and headless stderr output |
...
| `ASTRA_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `ASTRA_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `ASTRA_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `ASTRA_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `ASTRA_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `ASTRA_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `ASTRA_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.astra/config.toml` | Main configuration file |
| `~/.astra/pager.toml` | TUI appearance configuration |
...
| `~/.astra/lsp.json` | LSP server configuration (user-scoped) |
| `~/.astra/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.astra/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some settings can be set per-project by placing files in `.astra/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.astra/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.astra/config.toml`) |
| `.astra/skills/` | Project-specific skills |
| `.astra/hooks/` | Project-specific lifecycle hooks |
| `.astra/agents/` | Project-specific agent definitions |
| `.astra/lsp.json` | LSP server configuration |
| `.astra/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.astra/lsp.json` | All projects |
| Project | `.astra/lsp.json` | Current repository |
...
1. **Project** — `.astra/lsp.json`
2. **User** — `~/.astra/lsp.json`
