...
Astra is a terminal-based AI coding assistant from Astra. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
Once authenticated, Astra presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, Astra's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. Astra reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, Astra asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `astra --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.astra/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `astra -c`
...
- **Agent messages** -- Astra's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- Astra's reasoning process (collapsible)
...
astra "fix the failing auth test and run it"

# Initial prompt in a new git worktree. Use --worktree=<name> (with `=`) so the
# prompt isn't swallowed as the worktree name — `astra -w "refactor module X"`
# would treat "refactor module X" as the worktree label, not the prompt.
astra --worktree=feat "refactor module X"

# Base the worktree on a specific branch (e.g. main) instead of the current HEAD:
astra -w --ref main "implement feature from main"


# Start in a specific project directory
astra --cwd ~/projects/my-app
...
astra --resume <session-id>

# Continue the most recent session
astra -c

# Experimental scrollback-native render mode. Sticky: plain `astra` reopens in
...
Run Astra non-interactively for scripting, CI/CD, and automation:
...
```bash
astra -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. Astra reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.astra/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. Astra also reads `CLAUDE.md` files for compatibility.
