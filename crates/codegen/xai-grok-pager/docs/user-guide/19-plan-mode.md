...
The plan is written to `plan.md` inside the session directory (`~/.astra/sessions/<cwd>/<session-id>/plan.md`, where `<cwd>` is an encoded directory name, not the literal path).
