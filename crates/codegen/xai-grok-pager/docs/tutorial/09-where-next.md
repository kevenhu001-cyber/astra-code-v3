...
- **Ask Astra itself** — it can read its own user guide and set itself up.
  Try: "How do I run you in CI?" or "add an MCP server for GitHub".

## Good habits

- Sessions save automatically. Resume the latest with `astra -c`, or pick
  one with `/resume` (`Ctrl+S`).
- Long session getting slow? `/compact` frees context; `/context` shows
  where it's going.
- Automate anything: `astra -p "summarize new TODOs" --output-format json`
  runs headless — great for scripts and CI.
- Stay current with `astra update`; see what changed with `/release-notes`.
