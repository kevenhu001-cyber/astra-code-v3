...
Astra asks before doing anything risky — and can plan before it codes.

## Permissions

When Astra wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Astra explores the
...
`s` requests changes — Astra iterates until you're happy, then implements.
...
background — Astra keeps working and you're notified when it finishes
