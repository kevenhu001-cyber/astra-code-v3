#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.astra` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.astra` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.astra` reads).
    #[test]
