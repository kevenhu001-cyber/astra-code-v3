...
//!
//! Env sandboxing follows this crate's `serial(ASTRA_HOME)` idiom; note
...
//! `GROK_LEADER_SOCKET`, `ASTRA_HOME`) for a real agent's whole lifetime, and
...
struct PagerLeaderCluster {
    ...
    /// agent on the same ASTRA_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(ASTRA_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let env = vec![
            crate::test_util::EnvVarGuard::set("ASTRA_HOME", grok_home.path()),
            ...
            crate::test_util::EnvVarGuard::set("ASTRA_TELEMETRY_ENABLED", "false"),
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same ASTRA_HOME when the next generation's
