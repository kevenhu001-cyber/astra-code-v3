...
fn fit_docs_ask_grok_tip(docs_path: &str, width: usize) -> String {
    ...
    let long =
        format!("Tip · Ask Astra about the docs ({docs_path}) — e.g. \"how do I set up MCP?\"");
    if long.width() <= width {
        return long;
    }
    let short = format!("Tip · Ask Astra about the docs · {docs_path}");
...
mod doc_picker_tip_tests {
    ...
    fn fit_docs_tip_prefers_path_and_never_overflows() {
        ...
        let long =
            format!("Tip · Ask Astra about the docs ({path}) — e.g. \"how do I set up MCP?\"");
        let short = format!("Tip · Ask Astra about the docs · {path}");
    ...
    fn doc_picker_renders_tip_with_path() {
        ...
        assert!(
            all.contains("Tip") && all.contains("Ask Astra"),
            "missing tip footer:\n{all}"
        );
        assert!(
