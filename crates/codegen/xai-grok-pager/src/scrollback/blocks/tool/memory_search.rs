...
fn shorten_path(path: &str) -> &str {
    ...
    if let Some(rest) = path.strip_prefix(&memory_prefix) {
        // Windows joins produce `\` separators; accept both spellings.
...
**File:** /root/.astra/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.astra/memory/MEMORY.md");
...
**File:** /root/.astra/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.astra/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
