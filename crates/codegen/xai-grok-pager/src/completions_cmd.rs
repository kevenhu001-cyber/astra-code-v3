...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "astra" name (matches historical
    // behavior and what the installers + docs expect).
    let mut cmd = PagerArgs::command().name("astra");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "astra", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "astra", &mut buf);
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            // Brand-agnostic: matches "<bin>-command-$line[2]:" for any bin name.
            r#"-command-$line[2]:""#,
            r#"-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("astra");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "astra", &mut buf);
