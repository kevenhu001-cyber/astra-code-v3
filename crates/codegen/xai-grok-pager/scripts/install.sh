...
LEGACY_SCOPE="https://accounts.topodrive.top/sign-in"
AUTH_SOURCE=""
...
if [ "$os" = "windows" ]; then
    ...
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded astra failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.astra/bin and ~/.astra/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/astra"
