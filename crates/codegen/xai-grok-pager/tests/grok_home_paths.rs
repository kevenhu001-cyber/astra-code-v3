//! `ASTRA_HOME` override tests in an isolated binary so `grok_home()`'s
...
fn disk_usage_run_creates_no_grok_home() {
    ...
    unsafe {
        std::env::set_var("ASTRA_HOME", &ghost);
    }
