...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    if let Ok(home) = std::env::var("ASTRA_HOME").or_else(|_| std::env::var("GROK_HOME"))
...
Reads [voice] from ~/.astra/config.toml unless --config PATH is set.
"#
    );
}
